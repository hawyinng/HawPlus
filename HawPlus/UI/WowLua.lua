...
WowLua_DB = {
	pages = {
		[1] = {name = format(L.NEW_PAGE_TITLE, 1), content = "", untitled = true, checked = false, runtime = 0.05, StartTime = 0}
	},
	currentPage = 1,
	untitled = 2,
    fontSize = 15,
...
function WowLua:Button_Config()
    InterfaceOptionsFrame_OpenToCategory("HawPlus")
end
...
function WowLua:OnSizeChanged(frame)
	...
	if WowLuaFrameResizeBar and false then
		...
		WowLuaFrameResizeBar:ClearAllPoints()
		WowLuaFrameResizeBar:SetPoint("TOPLEFT", parent, "BOTTOMLEFT", 15, newPoint)
...
function WowLua:ResizeBar_OnUpdate(frame, elapsed)
	...
	frame:SetPoint("TOPLEFT", parent, "BOTTOMLEFT", 15, newPoint)
...
local function canScroll(scroll, direction)
	...
	local num = scroll:GetNumMessages();
	if ( direction == "up" ) 
		then
		return false;
