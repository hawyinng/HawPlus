StaticPopupDialogs["NoFireHack"] = {text = "|cFFFF4500HawPlus_classic提示：|r\n客户端未解锁或解锁失败!\n解锁后重载插件或小退.\n如需不解锁进行游戏,请禁用本插件.",	button1 = "OK",	timeout = 0, exclusive = 1,	whileDead = 1, hideOnEscape = nil};
HawPlus_classicLuaList = {};
HawPlus_classichelper = {Queuingtime = 5, Queuingchecked = false, Loadscript = true, wowlua = true, Insert = false};
...
local ResetEditbox = function()
	if firstone then
		...
		firstone = false
		for name, _ in pairs(HawPlus_classicLuaList) do
			if not tContains(luaScript,name) then
				HawPlus_classicLuaList[name] = nil
			end
		end
	end
end
local HawPlus_classicList = {}
local menu = HawPlus_classicScriptMenu("MSSM")
menu:SetMenuRequestFunc(HawPlus_classicList, "OnMenuRequest")
function HawPlus_classicList:OnMenuRequest(level, value, menu)
	HawPlus_classicLoadscript()
	if level == 1 then
		menu:AddLine("text", "|cff00ffffHawPlus_classic脚本文件列表|r", "ToggleButton",1,"ToggleState",not HawPlus_classichelper.Loadscript, "isTitle",1,
		"ToggleButtonFun",function()
			HawPlus_classichelper.Loadscript = not HawPlus_classichelper.Loadscript;
			menu:Refresh(level);
		end,
		"tooltipTitle","HawPlus_classic脚本说明", "tooltipText","本列表自动实时识别文件内所有.lua脚本文件。文件夹目录:\n" .. GetWoWDirectory() .. "\\HawPlus_classic脚本\\\n特点：\n1.脚本在游戏外编写。\n2.所有角色共用脚本。\n3.脚本与插件及游戏独立储存。")
		if HawPlus_classichelper.Loadscript then
			menu:AddLine("line",1, "LineBrightness",1, "LineHeight",10);
			if #luaScript >0 then
				for page, name in ipairs(luaScript) do
					menu:AddLine("text",name, "arg1",name, "value",name, "checked",HawPlus_classicLuaList[name].checked, "hasArrow",1,
					"func", function(arg1)
						if HawPlus_classicLuaList[name].checked then
							HawPlus_classicLuaList[name].checked = false;
						else
							HawPlus_classicLuaList[name].checked = true;
						end
						menu:Refresh(level);
					end);
		...
		menu:AddLine("line",1, "LineBrightness",1, "LineHeight",10);
		menu:AddLine("text", "|cff00ffff游戏内脚本编辑器列表|r", "ToggleButton",1,"ToggleState",not HawPlus_classichelper.wowlua, "isTitle",1,
		"ToggleButtonFun",function()
			HawPlus_classichelper.wowlua = not HawPlus_classichelper.wowlua;
			menu:Refresh(level);
		end,
		"tooltipTitle","编辑器说明", "tooltipText","本列表需要在游戏内自行新建编写。\n特点：\n1.脚本在游戏内编写。\n2.所有角色独立脚本。\n3.脚本储存于客户端的人物配置文件内:\n" .. GetWoWDirectory() .. "\\WTF\\")
		menu:AddLine("line",1, "LineBrightness",1, "LineHeight",10);
		if HawPlus_classichelper.wowlua then
	...
	elseif level == 2 then
		if HawPlus_classichelper.Loadscript and type(value) == "string" then
			...
			menu:AddLine("text","脚本调试", "icon","Interface\\ICONS\\INV_Scroll_05", "arg1",value, "func",function(name)
				RunScript(ReadFile(GetWoWDirectory() .. "\\HawPlus_classic脚本\\" .. name));
			end);
			menu:AddLine("line",1, "LineBrightness",0.3, "LineHeight",6)
			menu:AddLine("text","脚本延时", "icon","Interface\\ICONS\\INV_Elemental_SpiritofHarmony_2", "hasEditBox", 1, "editBoxText", HawPlus_classicLuaList[value].runtime, "editBoxArg1", HawPlus_classicLuaList[value],"editBoxArg2", self.text,
		...
		if HawPlus_classichelper.wowlua and type(value) == "number" then
...
SafeQueue:SetScript("OnEvent", function()
	for i=1, GetMaxBattlefieldID() do
		...
		if status == "confirm" then
			...
			-- if HawPlus_classichelper.Queuingchecked and FireHack and mapName then
				-- print("排队助手:" .. HawPlus_classichelper.Queuingtime .. "秒后自动进入" .. mapName)
			-- end
			break;
...
local minmap = {}
local minmapmenu = HawPlus_classicScriptMenu("MSSM")
...
function minmap:OnMenuRequest(level, value, minmapmenu)
	...
	if level == 1 then
		...
		minmapmenu:AddLine("line",1, "LineBrightness",1, "LineHeight",10);
		minmapmenu:AddLine("text","技能插入", "arg1",HawPlus_classichelper, "checked",HawPlus_classichelper.Insert, "func", function(arg1)
			if arg1.Insert then
				arg1.Insert = false;
			else
				arg1.Insert = true;
		...
		minmapmenu:AddLine("text","|cFF00FF00自动进入竞技场|r", "hasEditBox",1, "editBoxText",HawPlus_classichelper.Queuingtime, "editBoxArg1", HawPlus_classichelper, "editBoxArg2", self.text,
		...
		"arg1",HawPlus_classichelper, "hasArrow",1, "checked",HawPlus_classichelper.Queuingchecked,
		"func", function(arg1)
...
local AutoScriptOnUpdate = CreateFrame("Frame");
	...
	if HawPlus_classichelper.Insert then
	...
	if HawPlus_classichelper.Queuingchecked and FireHack then
		local i = SafeQueue.index
		local Queuingtime = SafeQueue.time
		if i and Queuingtime and GetTime()- Queuingtime > HawPlus_classichelper.Queuingtime + SafeQueue.Waittime then
...
function HawPlus_classicLoadscript()
	wipe(luaScript);
	wipe(wowluaScript)
	luaScript = {("" .. "\\HawPlus_classic脚本\\*.lua")}
	if #luaScript == 1 and luaScript[1] == "" then
		luaScript[1] = nil
	else
		for i=1, #luaScript do
			if not HawPlus_classicLuaList[luaScript[i]] then
				HawPlus_classicLuaList[luaScript[i]] = {checked = false, runtime = 0.05}
			end
			if HawPlus_classicLuaList[luaScript[i]].checked then
				luaScript[luaScript[i]] = {StartTime = 0,
				runtime = HawPlus_classicLuaList[luaScript[i]].runtime,
				content = (ReadFile(GetWoWDirectory() .. "\\HawPlus_classic脚本\\" .. luaScript[i]))};
...
function msStartStopScript(name,switch)
	...
	if HawPlus_classicLuaList[name] then
...
BINDING_HEADER_HawPlus_classic = "HawPlus_classic";
BINDING_NAME_HawPlus_classicShow = "显示/隐藏插件图标";
BINDING_NAME_HawPlus_classicOnce = "插件运行一次";
BINDING_NAME_HawPlus_classicStartStop = "启动/停止插件运行";
BINDING_NAME_HawPlus_classicStart = "启动插件运行";
BINDING_NAME_HawPlus_classicStop = "停止插件运行";

local ScenarioButton = CreateFrame("Button", "ScenarioButton", UIParent);
	ScenarioButton:SetWidth(50)
	ScenarioButton:SetHeight(50)
	...
	--ScenarioButton:SetNormalTexture("Interface\\Icons\\Ability_Mage_WintersGrasp")
	ScenarioButton:SetNormalTexture(134328)
...
local ButtonBackground = ScenarioButton:CreateTexture("ButtonBackground", "BACKGROUND");
	...
	ButtonBackground:SetSize(80,80)
...
SpellTargettext:SetText("HawPlus_classic 13.0\n升级技能插入模块,开关:\"Shift+鼠标右键\".")
