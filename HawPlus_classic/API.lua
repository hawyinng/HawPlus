...
function msISC(spell, target, move)
	...
	local spellname, _, _, castTime, _, _, spellId = GetSpellInfo(spell);
	...
	if (target) then
		...
		if (not isInRange) then
			local _, _, _, _, minRange, maxRange, _ = GetSpellInfo(spell);
...
function msGBB(buff, unit)
	...
	buff = tonumber(buff)
	for i = 1, 40 do
		local curName1, _, _, _, _, _, _, _, _, _, curId1 = UnitBuff(unit, i);
		local curName2, _, _, _, _, _, _, _, _, _, curId2 = UnitDebuff(unit, i);
