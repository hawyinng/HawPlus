...
function HawPlus_classicScriptMenu(DROPDOWNMENU_NAME_PREFIX)
	...
	local DROPDOWNMENU_INSET_HORIZONTAL = 12 -- Menu horizontal edge size	菜单水平边缘大小
	local DROPDOWNMENU_INSET_VERTICAL = 15 -- Menu vertical edge size		菜单垂直边缘大小
	local DROPDOWNMENU_BUTTON_HEIGHT = 18 -- Menu button height				菜单按钮高度
	local DROPDOWNMENU_HEADER_HEIGHT = 20 -- Menu header height				菜单标题高度
	local DROPDOWNMENU_BUTTON_WIDTH_MIN = 50 -- Menu button minimum width	菜单按钮最小宽度
	local DROPDOWNMENU_SEPARATOR_HEIGHT = 6 -- Menu separator height		菜单分隔高度
	local DROPDOWNMENU_TEXT_INSET = 2 -- Menu text inset					菜单插入文本
	local DROPDOWNMENU_COLUMN_SPACING = 6 -- Menu column spacing			菜单栏间距
	local DROPDOWNMENU_FRAME_SPACING = 7 -- Menu frame spacing				菜单帧间距
	local DROPDOWNMENU_RIGHT_EXTRA = 6 -- If none of the menu buttons has arrow or color swatch, append this extra space to the right edge	如果菜单按钮没有箭头或色块，添加额外的空间的右边缘
	...
	local _,BuildInfo = GetBuildInfo()
	if tonumber(BuildInfo)>50000 then
		return
	end
	...
	-- Toggle Button
		if info.ToggleButton then
			if not self.ToggleButton then
				...
				toggleBtn:SetPoint("TOPLEFT", self, "TOPLEFT", 0,0)--3, - 2)
			...
			self.ToggleButton:SetPoint("TOPLEFT", self, "TOPLEFT", ToggleX, ToggleY+1)--3 + ToggleX, - 2 + ToggleY);
		else
	...
	local function CreateDropDownMenu(level)
		...
		MenuCloseButton:SetWidth(30)
		MenuCloseButton:SetHeight(30)
